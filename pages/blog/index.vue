<template>
  <!-- Root wrapper with overflow-x-hidden to prevent layout shifts and silver bar issues -->
  <div class="bg-[#cdcfd1] min-h-screen font-sans selection:bg-[#db961f] selection:text-black flex flex-col overflow-x-hidden">

    <!-- ==========================================
      1. HEADER / HERO (Dark - Aligned with Header)
    =========================================== -->
    <header class="relative pt-32 pb-32 overflow-hidden bg-[#090a0a]">
      ...
      <!-- Standard container used to match Header and About page logic -->
      <div class="container mx-auto px-6 relative z-10 flex flex-col md:flex-row justify-between items-center gap-6" data-aos="fade-down">
        <div class="text-center md:text-left">
          ...
          <h1 class="text-3xl md:text-4xl lg:text-5xl font-display font-bold tracking-tight text-white mb-2 leading-tight">
    ...
    =========================================== -->
    <main class="flex-grow container mx-auto px-6 py-16 w-full space-y-12 relative z-20 bg-[#cdcfd1] -mt-16 rounded-t-[3rem] shadow-[0_-20px_50px_rgba(0,0,0,0.2)]">
      ...
      <section class="bg-white/50 backdrop-blur-md rounded-2xl p-8 border-l-4 border-[#db961f] shadow-lg border-y border-r border-white/40" data-aos="fade-up">
        <h2 class="text-xl font-bold text-[#090a0a] mb-3 uppercase tracking-wider">Philosophy: Systems Over Signals</h2>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
        ...
        <div class="lg:col-span-4 flex flex-col space-y-6" data-aos="fade-right">
          ...
          <div class="space-y-3 flex-grow">
            ...
            >
              <div class="flex items-center">
                ...
                <div>
                  <h4 class="font-bold text-sm md:text-base text-[#090a0a] transition-colors group-hover:text-[#db961f]">{{ cat.title }}</h4>
          ...
          <div class="bg-white/40 p-6 rounded-2xl border border-white/50 shadow-sm">
            <h4 class="text-[11px] font-bold uppercase tracking-[0.2em] text-[#db961f] mb-4 font-mono">Index Parameters</h4>
          ...
          <div class="bg-[#090a0a] rounded-3xl border border-[#090a0a]/20 h-full p-8 md:p-10 flex flex-col relative overflow-hidden transition-all duration-300 shadow-2xl group">
            ...
            <div class="relative z-10 flex flex-col h-full">
                  ...
                  <template v-for="(article, i) in activeCategory.articles" :key="article.slug || i">
                      ...
                      <div class="flex items-start gap-4">
                        ...
                        <div class="flex-grow">
                          ...
                          <div class="flex items-center justify-between text-[10px] font-mono uppercase tracking-widest border-t border-white/5 pt-3 mt-2">
                            <span class="text-[#db961f] group-hover/article:translate-x-1 transition-transform flex items-center gap-1 font-bold">Read Article <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg></span>
                    ...
                    >
                      <div class="flex items-start gap-4">
                        ...
                        <div class="flex-grow">
                          <h5 class="text-white/70 font-medium mb-1 leading-snug pr-4">{{ article.title }}</h5>
              ...
              <div class="mt-8 pt-6 border-t border-white/10">
                ...
                <NuxtLink to="/services" class="inline-flex items-center gap-2 text-sm font-bold text-[#cdcfd1] hover:text-[#db961f] transition-colors group/link">
        ...
        <div class="lg:col-span-3 h-full" data-aos="fade-left">
          <div class="bg-white/60 text-[#090a0a] rounded-3xl shadow-xl border border-white/50 p-8 h-full flex flex-col justify-between relative overflow-hidden">
            ...
            <div class="mt-8 pt-6 border-t border-[#090a0a]/10 relative z-10 text-center">
              <p class="font-bold uppercase tracking-[0.2em] text-[#db961f] text-sm">"Systems over Signals"</p>
            </div>
          </div>
        </div>
      ...
      <!-- Bottom Row: Pure CSS Custom Analytics (Dark Panels) -->
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-8">
        ...
        <div class="bg-[#090a0a] border border-[#090a0a]/20 shadow-xl p-8 rounded-3xl" data-aos="fade-up" data-aos-delay="100">
          ...
          <div class="h-48 flex items-end justify-between gap-2 md:gap-4 relative border-b border-white/10 pb-2">
            <div v-for="article in activeCategory.articles" :key="article.shortName" class="flex-1 flex justify-center items-end gap-1 h-full group">
      ...
      <section class="mt-16 bg-[#090a0a] rounded-3xl p-10 md:p-14 text-center border border-[#090a0a]/20 shadow-2xl relative overflow-hidden" data-aos="zoom-in">
        ...
        <h2 class="text-3xl md:text-4xl font-display font-bold text-white mb-4 relative z-10 uppercase tracking-tighter">Ready to Build?</h2>
    ...
    <Transition name="fade">
      <div v-if="isModalOpen" class="fixed inset-0 bg-black/80 z-[300] flex items-center justify-center p-4 backdrop-blur-sm">
        <div class="bg-[#cdcfd1] rounded-3xl shadow-2xl max-w-md w-full p-8 relative border border-white/50">
          ...
          <div class="w-12 h-12 bg-[#090a0a] rounded-xl flex items-center justify-center mb-6 shadow-lg"><svg class="w-6 h-6 text-[#db961f]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg></div>
          <h3 class="text-2xl font-bold text-[#090a0a] mb-4 uppercase tracking-tighter">About this Dashboard</h3>
          <p class="text-sm text-[#090a0a]/70 mb-6 leading-relaxed">This interface visualizes the information architecture of the Yuktrix Blog. It transforms static research into an interactive "Explorer" for quantitative trading research.</p>
          <button @click="isModalOpen = false" class="w-full py-3 bg-[#090a0a] text-white rounded-xl font-bold uppercase tracking-widest text-xs hover:bg-[#db961f] transition-colors">Acknowledge</button>
        </div>
      </div>
    </Transition>
...
const blogData = {
  metadata: {
    title: "Trading Research Blog | Yuktrix",
    keywords: ["Systemized trading", "Quantitative validation", "Automation", "Quant research", "Market structure"]
  },
  categories: [
    {
      id: "systemized",
...
const donutSegments = computed(() => {
  ...
  return blogData.categories.map(cat => {
    ...
    const segment = { length: length - 2, offset: -currentOffset, color: cat.color };
